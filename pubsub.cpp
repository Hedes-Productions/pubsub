#include "../aws-iot-device-sdk-cpp-v2/samples/utils/PipeConnection.h"
#include <chrono>
#include <iostream>
#include <string>
#include <thread>
...
#define PIPE_NAME "\\\\.\\pipe\\pipebridges"
#define BUFSIZE 1024

std::string getRobotLocationRotation(DWORD &dwWritten, TCHAR chBuf[], HANDLE hPipe)
{
    while (true)
    {
        ...
        if (hPipe == INVALID_HANDLE_VALUE)
        {
            ...
            // std::cerr << "Creating pipe server" << std::endl;
            hPipe = CreateFile(PIPE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
            std::cerr << "Creating pipe server" << dwError << std::endl;

            if (dwError == ERROR_FILE_NOT_FOUND)
            {
                std::cerr << "Named pipe server is not running or the pipe does not exist." << std::endl;
                hPipe = CreateFile(PIPE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
            }
        }
        else

        {
            ...
            if (dwError == ERROR_PIPE_BUSY) // Server is not available, but the pipe exists
            {
                std::cerr << "Named pipe server is busy or not available." << std::endl;
            }
            else if (dwError == ERROR_FILE_NOT_FOUND) // Server is not running or pipe does not exist
            {
                std::cerr << "Named pipe server is not running or the pipe does not exist." << std::endl;
            }
            else if (dwError == ERROR_BROKEN_PIPE)
            {
                std::cerr << "Named pipe connection has been terminated by the server." << std::endl;
                CloseHandle(hPipe);
                hPipe = CreateFile(PIPE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
                std::cerr << hPipe << std::endl;
            }
            ...
            {
                std::wstring messageToSend = L"Message from basic-pub-sub client";
                DWORD dwMessageSize = static_cast<DWORD>(sizeof(wchar_t) * messageToSend.size());
                if (!WriteFile(hPipe, messageToSend.c_str(), dwMessageSize, &dwWritten, NULL))
                {
                    std::cerr << "Failed to write data to named pipe: " << GetLastError() << std::endl;
                }
                else
                {
                    std::cout << "Message sent to robotController" << std::endl;
                }

                if (!ReadFile(hPipe, chBuf, BUFSIZE * sizeof(TCHAR), &dwWritten, NULL))
                {
                    std::cerr << "Failed to read data from named pipe: " << GetLastError() << std::endl;
                }
                else
                {
                    // Convert TCHAR buffer to string
#ifdef UNICODE
                    std::wstring receivedMessage(chBuf, dwWritten / sizeof(TCHAR));
                    std::wcout << "Received message from robotController: " << receivedMessage << std::endl;
#else
                    std::string receivedMessage(chBuf, dwWritten / sizeof(TCHAR));
                    std::cout << "Received message from robotController: " << receivedMessage << std::endl;
#endif
                }
            }

            std::cout << "Named pipe client connected" << dwError << std::endl;
        }
 ...
 */
 ...
 #include "../../utils/PipeConnection.h"
...
{
        DWORD dwWritten;
        TCHAR chBuf[BUFSIZE + 1] = {0};
        HANDLE hPipe = INVALID_HANDLE_VALUE;
    while (true)
    {
            std::string robotLocationRotation = getRobotLocationRotation(dwWritten, chBuf, hPipe);
        std::cout << "Robots position " << robotLocationRotation << std::endl;
        std::string messagePayload = robotLocationRotation;
        ...
        connection->Publish(cmdData.input_topic.c_str(), AWS_MQTT_QOS_AT_LEAST_ONCE, false, payload,
        onPublishComplete);
        ++publishedCount;
 ...
 int main(int argc, char *argv[])
{
    ...
    if (connectionCompletedPromise.get_future().get())
    {
        ...
        //}]
        uint32_t publishedCount = 0;
        sendData(cmdData, connection, receiveMutex, receiveSignal, receivedCount, publishedCount);
